...
namespace Chess
{
    class Program
    {
        static void Main()
        {
            ...
            Console.Write("Шахматaная задача с определением угрозы королю, шахматное поле представляет собой поле 8 x 8 клеток,\n так что числа в вводе вводите от 1 до 8 включительно \n");
            ...
            if (KorolX < 1 || KorolX > 8 || KorolY < 1 || KorolY > 8) // король за полем или на неположеном месте?
            {
            ...
            if (Slonx < 1 || Slonx > 8 || Slony < 1 || Slony > 8) // слон за полем или на неположеном месте?
            {
                Console.WriteLine("Ошибка! Фигура за полем!");
                return;
            }
            ...
            if (Ladyax < 1 || Ladyax > 8 || LadyaY < 1 || LadyaY > 8) // ладья за полем или на неположеном месте?
            {
                Console.WriteLine("Ошибка! Фигура за полем!");
                return;
            }
            ...
            if ((Slonperedladya)) { Console.WriteLine("слон закрывает короля от шаха ладьи, шаха нет"); } Main();
            if ((ladyaperedslon)) { Console.WriteLine("ладья закрывает короля от шаха слона, шаха нет"); } Main();
            if (((l & !s) && (!Slonperedladya))) { Console.WriteLine("шах от ладьи"); } Main();
            if (((s & !l) && (!ladyaperedslon))) { Console.WriteLine("шах от слона"); } Main();
            if ((l && s)) { Console.WriteLine("шах от слона и ладьи одновременно"); } Main();
            if ((k)) { Console.WriteLine("нет шаха"); } Main();
